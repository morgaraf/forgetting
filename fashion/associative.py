...
class AssociativeMemory(object):
    def __init__(self, n: int, m: int,
        ...
        """
        print(f'{{iota: {iota}, kappa: {kappa}, xi: {xi}, sigma: {sigma}, max_col_weight: {max_col_weight}}}')
        self._n = n
        self._m = m+1
        self._t = xi
        self._absolute_max = 1023
        self._max_col_weight = max_col_weight
        self._sigma = sigma*m
        self._iota = iota
        self._kappa = kappa
        ...
        self._relation = np.zeros((self._m, self._n), dtype=int)
        # Iota moderated relation
        self._iota_relation = np.zeros((self._m, self._n), dtype=int)
    ...
    def abstract(self, r_io) -> None:
        if self._max_col_weight is None:
            self._relation = np.where(
                self._relation == self.absolute_max_value, 
                self._relation, self._relation + r_io)
        else:
            # Implement explicit forgetting logic based on maximum column weight
            for j in range(self.n):
                active_cells = np.where(r_io[:, j])[0]
                if len(active_cells) == 0:
                    continue
                
                # Calculate current sum of weights in the column
                S = np.sum(self._relation[:, j])
                
                for cell in active_cells:
                    if S < self._max_col_weight:
                        self._relation[cell, j] += 1
                        S += 1
                    else:
                        # Column is full (S >= W)
                        self._relation[cell, j] += 1
                        
                        # Find other cells to decrement
                        weights = self._relation[:, j].copy()
                        weights[cell] = 0 # Exclude the cell we just incremented
                        
                        available_indices = np.where(weights > 0)[0]
                        if len(available_indices) > 0:
                            # Nueva fórmula de probabilidad del profesor: P = 1 - peso/suma(pesos)
                            N_avail = len(available_indices)
                            if N_avail == 1:
                                probs = np.array([1.0])
                            else:
                                S_avail = np.sum(weights[available_indices])
                                p_raw = 1.0 - (weights[available_indices] / S_avail)
                                # Normalizamos para que la suma sea 1 (la suma de p_raw es N_avail - 1)
                                probs = p_raw / (N_avail - 1.0)
                            
                            # Randomly pick a cell to decrement based on inverted distribution
                            cell_to_decrement = np.random.choice(available_indices, p=probs)
                            self._relation[cell_to_decrement, j] -= 1
                        else:
                            # If no other cell has weight > 0, we decrement the same cell
                            # to maintain the column sum at max_col_weight.
                            self._relation[cell, j] -= 1
        self._updated = False
